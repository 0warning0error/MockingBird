...
from models.synthesizer.preprocess_audio import preprocess_general, extract_emo, preprocess_general_npz
...
new_data_info = {
    "aidatatang_200zh": {
        "subfolders": ["corpus/train"],
        "trans_filepath": "transcript/aidatatang_200_zh_transcript.txt",
        "speak_func": preprocess_general_npz
    },
    "aidatatang_200zh_s": {
        "subfolders": ["corpus/train"],
        "trans_filepath": "transcript/aidatatang_200_zh_transcript.txt",
...
def preprocess_dataset(datasets_root: Path, out_dir: Path, n_processes: int,
                           ...
                           dataset: str, emotion_extract = False, encoder_model_fpath=None, **kwargs):
    ...
    assert transcript_dirs.exists(), f"{transcript_dirs} not exist."
    ...
    speaker_dirs = tuple(chain.from_iterable(input_dir.glob("*") for input_dir in input_dirs))
...
def preprocess_dataset_new(datasets_root: Path, out_dir: Path, n_processes: int,
                           skip_existing: bool, hparams, no_alignments: bool, 
                           dataset: str, emotion_extract = False, encoder_model_fpath=None, **kwargs):
    dataset_info = new_data_info[dataset]
    # Gather the input directories
    dataset_root = datasets_root.joinpath(dataset)
    input_dirs = [dataset_root.joinpath(subfolder.strip()) for subfolder in dataset_info["subfolders"]]
    print("\n    ".join(map(str, ["Using data from:"] + input_dirs)))
    ...
    # Create a metadata file
    metadata_fpath = out_dir.joinpath("train_new.txt")
    ...
    job = Pool(n_processes).imap_unordered(func, speaker_dirs)
    buf_data = {}
    with metadata_fpath.open("a" if skip_existing else "w", encoding="utf-8") as metadata_file:
        for speaker_metadata in tqdm(job, dataset, len(speaker_dirs), unit="speakers"):
            if speaker_metadata is not None:
                buf_data.update(speaker_metadata)
        metadata_file.write(yaml.safe_dump(buf_data))
    

    
    mel_frames = sum(int(metadata[m][2]) for m in metadata)
    timesteps = sum(int(metadata[m][1]) for m in metadata)
    sample_rate = hparams.sample_rate
    hours = (timesteps / sample_rate) / 3600
    print("The dataset consists of %d utterances, %d mel frames, %d audio timesteps (%.2f hours)." %
          (len(metadata), mel_frames, timesteps, hours))
    print("Max input length (text chars): %d" % max(len(metadata[m][3]) for m in metadata))
    print("Max mel frames length: %d" % max(metadata[m][2] for m in metadata))
    print("Max audio timesteps length: %d" % max(metadata[m][1] for m in metadata)) 
...
def create_embeddings(synthesizer_root: Path, encoder_model_fpath: Path, n_processes: int):
    ...
    with metadata_fpath.open("r", encoding="utf-8") as metadata_file:
        metadata = tuple(line.split("|") for line in metadata_file)
